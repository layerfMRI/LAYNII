...
int show_help( void )
{
   printf(
      ...
      "\n"
      "    This program removes somt of the background noise in MP2RAGE ,\n"
      "    UNI images to make themn look like MPRAGE images,\n"
      "    This is dont without need ing the have the phase information  \n"
      "    This is following the paper O’Brien KR et al. (2014) \n"
      "    Robust T1-Weighted Structural Brain Imaging and Morphometry \n"
      "    at 7T Using MP2RAGE. PLoS ONE 9(6): \n"
      ...
      "\n"
      "       -help                 : show this help\n"
      "       -INV1                 : nii of the first inversion time (mandatory) \n"
      "       -INV2                 : nii of the second inversion time (mandatory) \n"
      "       -UNI                 : nii of MP2RAGE UNI image \n"
      "                               (I am expecting SIEMENS values 0-4094) \n"
      "       -beta  value          : regularization term  \n"
      "                               default is 0.2 \n"
      "\n");
   return 0;
}
...
int main(int argc, char * argv[])
{
   ...
   if( !finfi_1  ) { fprintf(stderr, "** missing option '-INV1'\n");  return 1; }
   if( !finfi_2  ) { fprintf(stderr, "** missing option '-INV2'\n");  return 1; }
   if( !finfi_3  ) { fprintf(stderr, "** missing option '-UNI '\n");  return 1; }
  ...
  for(int iz=0; iz<sizeSlice; ++iz){  
      for(int iy=0; iy<sizePhase; ++iy){
        for(int ix=0; ix<sizeRead-0; ++ix){
			 ...
			 /////////////////////////////////////////////////////////////////////////////////////////////
			 ////  scaling UNI to range of -0.5 to 0.5 as in the original paper                      /////
             /////////////////////////////////////////////////////////////////////////////////////////////
...
cout << " ############################################################# " << endl; 
  ...
  if( nifti_set_filenames(dddenoised, fout_1 , 1, 1) ) return 1;
  nifti_image_write( dddenoised );
  
  const char  *fout_2="Border_enhance.nii" ;
  if( nifti_set_filenames(phaseerror, fout_2 , 1, 1) ) return 1;
