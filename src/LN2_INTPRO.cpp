...
int show_help(void) {
    ...
    "\n"
    "Usage:\n"
    "     LN2_INTPRO -input file.nii -range 3 -type min \n"
    "\n"
    "Options:\n"
    "    -help   : Show this help\n"
    "    -input  : Nifti (.nii) for intensity projections. \n"
    "    -axis   : Axis of the projection. x=1, y=2, z=3. Default is '3'.\n"
    "              Projections over each axis can be generated by using '0'.\n"
    "    -type   : Projection type. Options are: 'min', 'max'. Default is 'min'.\n"
...
int main(int argc, char * argv[]) {
    ...
    std::string projection_type = "min";
    ...
    if (projection_type == "min" || projection_type == "max" ) {
    ...
    if ( projection_type == "max" ) {
        // --------------------------------------------------------------------
        if ( axis == 1 || axis == 0 ) {
            cout << "  Maximum intensity projection over x..." << endl;
            ...
            cout << endl;
            cout << "  Saving output..." << endl;
            save_output_nifti(fout, "maxip-x_range-"+tag_range.str(), nii_output, true);        
        }

        // --------------------------------------------------------------------
        if ( axis == 2 || axis == 0 ) {
            cout << "  Maximum intensity projection over y..." << endl;
            ...
            cout << endl;
            cout << "  Saving output..." << endl;
            save_output_nifti(fout, "maxip-y_range-"+tag_range.str(), nii_output, true);        
        }

        // --------------------------------------------------------------------
        if ( axis == 3 || axis == 0 ) {
            cout << "  Maximum intensity projection over z..." << endl;
            ...
            save_output_nifti(fout, "maxip-z_range-"+tag_range.str(), nii_output, true);        
        }
    }
