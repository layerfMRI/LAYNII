...
int main(int argc, char*  argv[]) {
    ...
    nifti_image* nii_id = copy_nifti_as_int32(nii_centroid);
    int32_t* nii_id_data = static_cast<int32_t*>(nii_id->data);
    ...
    for (int n = 0; n < nr_iter; ++n) {
        ...
        for (int i = 0; i != nr_voxels; ++i) {
            if (*(nii_centroid_data + i) != 0) {
                *(nii_step_data + i) = 1.;
                *(nii_dist_data + i) = 0.;
                *(nii_id_data + i) = *(nii_centroid_data + i);
            } else {
                *(nii_step_data + i) = 0.;
                *(nii_dist_data + i) = 0.;
            }
                ...
                if (*(nii_step_data + i) == grow_step
                    ...
                    if (ix > 0) {
                        ...
                        if (*(nii_rim_data + j) == 3) {
                            ...
                            if (d < *(nii_dist_data + j)
                                || *(nii_dist_data + j) == 0) {
                                *(nii_dist_data + j) = d;
                                *(nii_step_data + j) = grow_step + 1;
                                *(nii_id_data + j) = *(nii_id_data + i);
                            }
                        }
                    }
                    if (ix < end_x) {
        ...
        // save_output_nifti(fin1, "lloyd_id", nii_id, false);
