...
int show_help(void) {
    printf(
    "LN2_FRISGO :  This program aims to correct/mitigate the Fuzzy Ripple \n"
    "              artifacts in dual polarity 3D-EPI data.\n"
    "              \n"
    "\n"
    "Usage:\n"
    ...
    // "              changes of the fuzzy ripples across time. \n"
    // "              A Gaussian travelling window of averaging is used to \n"
    // "              to estimate the fuzzy ripples. Specify the value \n"
    // "              of the Gaussian size (float values) in units of TR. \n"
    // "              Make sure to use values that are bigger than you trial timing.\n"
    // "              The default value is '40'.\n"
    // "    -runwise: Doing the dual-polarity correction by assuming that the \n"
    // "              fuzzy ripples are identical for the entire run. \n"
    ...
    "        Magnetic Resonance in Med mrm.30489. <https://doi.org/10.1002/mrm.30489>\n"
    "\n"
    "\n");
    return 0;
}
...
int main(int argc, char * argv[]) {
    ...
    if (mode_spline) {
        ...
        if (size_time < 4) {
            cout << "***********************************************************" << endl;
            cout << " Less than 4 TRs, 3rd order spline fitting is not possible " << endl;
            cout << "***********************************************************" << endl;
            return 1; 
        }
        ...
        for (uint64_t i = 0; i < nr_voxels; ++i) {
            uint64_t t0 = nr_voxels*0, t1 = nr_voxels*1, t2 = nr_voxels*2;
        ...
        for (uint64_t i = 0; i < nr_voxels; ++i) {
            uint64_t t0 = nr_voxels*(size_time - 1), t1 = nr_voxels*(size_time - 2), t2 = nr_voxels*(size_time - 3);
           *(nii_smooth_data + t0 + i) = 0.5 * ( *(nii_data + t0 + i) + *(nii_data + t1 + i) );
           *(nii_smooth_data + t1 + i) = 0.5 * ( *(nii_data + t1 + i) + *(nii_data + t2 + i) );
        }
        ...
        for (uint64_t t = 2; t != size_time-2; ++t) {
            for (uint64_t i = 0; i < nr_voxels; ++i) {
                *(nii_smooth_data + nr_voxels*t + i) = 
                    - 0.0625 * ( *(nii_data + nr_voxels*(t-2) + i))
                    + 0.5625 * ( *(nii_data + nr_voxels*(t-1) + i))
                    + 0.5625 * ( *(nii_data + nr_voxels*(t)   + i))
    ...
    // // ========================================================================
    // // Prepare for run-wise and low pass Fuzzy Ripple correction
    // // ========================================================================
    // if (do_gaus || do_run) {
    //     if (do_verb) cout << "Preparing for runwise and low pass correction..." << endl;

    //     // Allocate memory for weight time series
    //     nifti_image* nii_weight = nifti_copy_nim_info(nii);
    //     nii_weight->nt = 1;
    //     nii_weight->datatype = NIFTI_TYPE_FLOAT32;
    //     nii_weight->nbyper = sizeof(float);
    ...
    //         cout << "Start smoothing loop... This might take a while" << endl;
