...
int show_help(void) {
    printf(
    "LN2_RIM_BORDERIZE: Hollow out filled segmentations aside from gray matter.\n"
    "                   This helps LayNii to compute faster in programs using\n"
    "                   'rim files' such as LN2_LAYERS will compute faster.\n"
    "\n"
    "Usage:\n"
    "    LN2_RIM_BORDERIZE -rim rim.nii\n"
    "    LN2_RIM_BORDERIZE -rim rim.nii -jumps 3\n"
    "\n"
    "Options:\n"
    "    -help   : Show this help.\n"
...
int main(int argc, char*  argv[]) {
    ...
    // Prepare output
    nifti_image* nii_borderized = copy_nifti_as_int32(nii_rim);
    int32_t* nii_borderized_data = static_cast<int32_t*>(nii_borderized->data);

    // Setting zero
    for (uint32_t i = 0; i != nr_voxels; ++i) {
        *(nii_borderized_data + i) = 0;
    }
    ...
    for (uint32_t ii = 0; ii != nr_voi; ++ii) {
        ...
        // Only hollow out non gray matter voxels
        if (*(nii_rim_data + i) == 1 || *(nii_rim_data + i) == 2) {
            ...
            if (switch_border) {
                *(nii_borderized_data + i) = *(nii_rim_data + i);
            }
        ...
        // Keep gray matter voxels intact
        if (*(nii_rim_data + i) == 3) {
            *(nii_borderized_data + i) = 3;
        }
    }

    // ------------------------------------------------------------------------

    save_output_nifti(fout, "borderized", nii_borderized, true, use_outpath);
