...
{
   printf(
      "LN_EXTREMETR: To find the TR time point that is extreme\n"
      "\n"
      "    This program tries to find the maximal/minimal value of a time series and writes out the timepoint of that TR vor every voxel  \n"
      "\n"
      "\n"
      "    basic usage: LN_EXTREMETR -file file.nii \n"
      "\n"
      "\n"
      "\n"
      "    options:\n"
...
int TR_min  = 0;
 ...
 cout << " runing also until here  5.... " << endl; 
  ...
  string outfilename_2 = prefix_2+filename_1 ;
   ...
   cout << "writing as = " << outfilename_1.c_str() << " and "<< outfilename_2.c_str() << endl; // finfi is: char *

  const char  *fout_1=outfilename_1.c_str() ;
  if( nifti_set_filenames(max_file, fout_1 , 1, 1) ) return 1;
  nifti_image_write( max_file );


  const char  *fout_2=outfilename_2.c_str() ;
  if( nifti_set_filenames(min_file, fout_2 , 1, 1) ) return 1;
