...
{
   printf(
      "LN_ZOOM: the program reduces the nii file to only include the rectangular space that is different from zero\n"
      "\n"
      "\n"
      "\n"
      "    basic usage: LN_ZOOM -mask mask.nii -input file_to_be_zoomed.nii \n"
      "\n"
      "\n"
      "\n"
      "    options:\n"
      "\n"
      "       -help               : show this help\n"
      "       -mask 			  : the mask file that determines the interesting area. E.g. the layer mask with one time point \n"
      "       -input  	          : the file that should be zoomed (e.g. with multiple time points)\n"
      "\n"
      "\n"
      "\n"
      "\n"
...
int main(int argc, char * argv[])
{
   ...
   if( !fin_1  ) { fprintf(stderr, "** missing option '-input'\n");  return 1; }
   ...
   if( !fin_2  ) { fprintf(stderr, "** missing option '-mask'\n");  return 1; }
   // read input dataset, including data 
...
cout << "reduction "  << (nim_file_1->nvox * zoomed_x_size  * zoomed_y_size * zoomed_z_size)  / (nim_file_1i->nx * nim_file_1i->ny * nim_file_1i->nz)  << endl;
  ...
  if( nifti_set_filenames(zoomed_file, fout_1 , 1, 1) ) return 1;
