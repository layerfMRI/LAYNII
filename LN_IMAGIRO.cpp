...
{
   printf(
      "LN_IMAGIRO : This program does the oposite of ORIGAMI, namely it unfolds stuff. "
      "\n"
      "   It generates a 3D matrix of columns and layers  \n"
      "\n"
      "    basic usage: LN_IMAGIRO -layer_file equi_dist_layers.nii -column_file columnar_coordinated.nii -data  data2unfold.nii \n"
      "\n"
      "\n"
      ...
      "\n"
      "       -help               	: show this help\n"
      "       -layer_file     		: nii file that contains layer  \n"
      "       -columns_file 		: nii file that contains columns,  \n"
      "       -data 		 		: data that should be unfolded.   \n"
...
int main(int argc, char * argv[])
{
   ...
   for( ac = 1; ac < argc; ac++ ) {
      if( ! strncmp(argv[ac], "-h", 2) ) {
         ...
         column_filename = argv[ac];  // no string copy, just pointer assignment 
      }
         ...
         data_filename = argv[ac];  // no string copy, just pointer assignment 
      }
      else {
   ...
   if( !column_filename  ) { fprintf(stderr, "** missing option '-column_file'\n");  return 1; }
   ...
   if( !nim_layers_r ) {
    ...
    if( !data_filename  ) { fprintf(stderr, "** missing option '-data'\n");  return 1; }
   // read input dataset, including data 
...
cout << "Input data is int 16  " << endl; 
  short  *nim_data_r_data = (short *) nim_data_r->data;
	  ...
	  for(int islice=0; islice<sizeSlice; ++islice){  
	      for(int iy=0; iy<sizePhase; ++iy){
	        for(int ix=0; ix<sizeRead; ++ix){
	          //#ask Rick about this if (*(nim_data_r_data  + nxyz *it +  nxy*islice + nx*ix  + iy  ) > 0 ) cout << *(nim_data_r_data  + nxyz *it +  nxy*islice + nx*ix  + iy  ) << endl; 
        		 *(nim_data_data  + nxyz *it +  nxy*islice + nx*ix  + iy  ) = (float) (*(nim_data_r_data  + nxyz *it +  nxy*islice + nx*ix  + iy  )) ;	
           } 
	    }
	  }
...
cout << "Input data is int 32  " << endl; 
  int  *nim_data_r_data = (int *) nim_data_r->data;
   ...
   cout << " Voxel size    " <<  dX << " x " <<  dY << " x "  <<  dZ  << endl; 
    ...
    cout << " datatye of Columns mask = " << nim_columns ->datatype << endl;
